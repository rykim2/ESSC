...
#' @param vector.output logical specifying whether or not the returned adjacency matrix should be returned as a vector of edge variables. Default is FALSE
#' @import Matrix
#' @import Rlab
...
# require(Rlab) require(Matrix)
stochastic.block = function(n, k = 2, P, sizes, random.community.assignment = c(FALSE, TRUE), vector.output = c(FALSE,
    TRUE)) {
    # Input: n = number of nodes k = number of blocks (default = 2) P = k x k matrix where the (i,j)th entry specifies
    # the probability of connection between community i and j sizes = an array of length k where the ith entry
    # specifies the size of the ith block. This must sum to n random.community.assignment = logical specifying whether
    # or not the labels are assigned randomly. If FALSE, labels are assigned in order starting with the upper left
    # corner of the matrix.  vector = logical specifying whether or not the returned adjacency matrix should be
    # returned as a vector of edge variables.  Output: A list containing three components: Adjacency: the adjacency
    # matrix of the generated network Membership: an n x 1 vector specifying community membership of each node
