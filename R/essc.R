...
#' }
#' @author James D. Wilsonchec
#' @examples
#' matrix_ie <- cbind(c(0.1, 0.01, 0.01), c(0.01, 0.1, 0.01), c(0.01, 0.01, 0.1))
#' net <- stochastic.block(n = 1000, k = 3, P = matrix_ie , sizes = c(300, 300, 400))
...
essc = function(Adj.Matrix, alpha, Null = c("Binomial", "Poisson"), Num.Samples = nrow(Adj.Matrix)) {
    ...
    return(list(Communities = Community, Background = Background, PValues = as.matrix(pvals), Indicator_Matrix = binary))
}
